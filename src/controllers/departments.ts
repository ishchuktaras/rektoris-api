...
// Classes
export const createDepartment = async (req: TypedRequestBody<DepartmentCreateProps>, res: Response): Promise<void> => {
  const data = req.body;
...
export async function getBriefDepartments(req: Request, res: Response): Promise<void> {
  try {
    const departments = await db.department.findMany({
      orderBy: {
        createdAt: "desc",
